...
def background_worker(job_id: str, action_type: str, args: dict):
    ...
    try:
        with redirect_stdout_to_stderr():
            ...
            elif action_type == "sync_recent_code":
                # รัน sync สำหรับโค้ดที่เพิ่งถูกแก้
                result = run_recent_code_sync(
...
@mcp.tool()
def sync_code_file(file_path: str, epic_key: str = "SCRUM-32") -> str:
    """
    Sync a single source code file to the Knowledge Graph (Neo4j & Vector DB) (ASYNC).
    This is useful for CI/CD pipelines or when you only need to sync one specific code file.
    Args:
        file_path: Absolute path to the source code file to sync.
        epic_key: The Jira Epic ticket ID to bind this code to (e.g., SCRUM-32).
    """
    job_id = f"codefile-{str(uuid.uuid4())[:6]}"
    ...
    return (
        f"🚀 Code File Sync Started! Job ID: {job_id}\n"
        f"Target Code File: {file_path}\n"
        f"Target Epic: {epic_key}\n\n"
        f"I am processing this code file in the background. "
        f"Please use `check_job_status('{job_id}')` to monitor the progress."
    )

@mcp.tool()
def sync_recent_code_updates(repo_path: str, hours: int = 24, epic_key: str = "SCRUM-32") -> str:
    """
    Sync all source code files modified within the last N hours (ASYNC).
    USE THIS TOOL WHEN: You need to update the graph with recently changed code files in a repository.
    Args:
        repo_path: Absolute path to the local git repository.
        hours: Number of hours to look back (e.g., 24).
        epic_key: The Jira Epic ticket ID (e.g., SCRUM-32).
    """
    job_id = f"recentcode-{str(uuid.uuid4())[:6]}"

    JOBS[job_id] = {
        "type": "sync_recent_code",
        "target": f"Last {hours}h in {repo_path}",
        "status": "PENDING",
        "start_time": time.strftime("%H:%M:%S")
    }
    ...
    return (
        f"🚀 Recent Code Sync Started! Job ID: {job_id}\n"
        f"Scanning Repo: {repo_path}\n"
        f"Looking back: {hours} hours\n"
        f"Target Epic: {epic_key}\n\n"
        f"Processing in the background. Use `check_job_status('{job_id}')` to monitor."
    )
