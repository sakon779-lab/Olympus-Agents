...
4. **SQLAlchemy Parameter Rule**: ALWAYS use `nullable=True/False`. NEVER use `Noneable`.
5. **Precision Edit Rule**: If you get an 'ambiguous' error, you MUST include the class name or unique surrounding lines in your `target_text`.
...
TOOL_SCHEMAS = {
    ...
    "write_file": {
        "required": ["content"], # ใส่กลับมาเพื่อบังคับให้มันเห็นว่าต้องส่ง
        "file_path": True,
        "args_schema": {
            "content": {
                "type": "string",
                "enum": ["LAST_CODE_BLOCK"], # บังคับว่าต้องมีค่านี้ค่าเดียวเท่านั้น!
                "description": "Must be exactly 'LAST_CODE_BLOCK'. Write content in Markdown above."
            }
        }
    },
    "append_file": {
        "required": [], # ลบ "content" ออก
        "file_path": True,
        "description": "Appends to file. Put the content in a Markdown block BEFORE this JSON. NO 'content' argument needed."
    },
    "edit_file": {
        "required": ["target_text"], # ลบ "replacement_text" ออก
        "file_path": True,
        "description": "Replaces text. Put the NEW code in a Markdown block BEFORE this JSON. NO 'replacement_text' argument needed."
    },
...
def execute_tool_dynamic(tool_name: str, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if tool_name in TOOL_SCHEMAS:
        ...
        raw_unknown = set(args.keys()) - valid_keys

        # กรอง 'content' และ 'replacement_text' ออกจากลิสต์ตัวแปรที่ผิด
        # เพื่อให้มันหลุดเข้าไปถึงสเต็ปการฉีดโค้ด (Middleware Injection)
        unknown_args = {k for k in raw_unknown if k not in ["content", "replacement_text"]}

        if unknown_args:
            # ถ้ามีตัวแปรอื่นที่ผิดจริงๆ (ไม่ใช่ content) ก็ยังด่าเหมือนเดิม
            return {
                "success": False,
...
def run_hephaestus_task(task: str, job_id: str = None, max_steps: int = 45):
    ...
    try:
        ...
        if 'dual_logger' in locals():
            dual_logger.close()
        # 🧹 [เพิ่มส่วนนี้] ให้ระบบ Auto-prune ขยะ Docker อัตโนมัติเมื่อจบงาน
        print("\n🧹 [System] Auto-cleaning dangling Docker images...")
        try:
            subprocess.run(["docker", "image", "prune", "-f"], check=False, stdout=subprocess.DEVNULL)
            print("✨ [System] Docker cleanup complete!")
        except Exception as e:
            print(f"⚠️ [System] Docker cleanup failed: {e}")
